...
#################### libiconv-1.15
cd libiconv-1.15
./configure -prefix $INSTALL_DIRECTORY --enable-static --disable-shared --with-pic 2>&1 | tee j_configure
make clean
make -j9 && make install 
if [ "$?" != "0" ]
then
    echo "Fail to build and install libiconv, check the error message below."
exit 1
fi
...
cd 00build-general
cmake .. -DCMAKE_BUILD_TYPE=Debug -DBUILD_SHARED_LIBS=OFF -DBUILD_STATIC=ON -DCMAKE_INSTALL_PREFIX=$INSTALL_DIRECTORY 2>&1 | tee j_cmake
make -j9 && make install 
if [ "$?" != "0" ]
then
    echo "Fail to build and install uchardet, check the error message below."
exit 1
fi

cd ../..
echo "uchardet installed."
...
cd 00build-general
cmake .. -DCMAKE_BUILD_TYPE=Debug -DBUILD_SHARED_LIBS=ON -DCMAKE_PREFIX_PATH=$INSTALL_DIRECTORY -DCMAKE_INSTALL_PREFIX=$INSTALL_DIRECTORY -DJBH_BUILD_FOR_GENERAL=ON  -DJBH_USE_EMBEDDED_UNICODE_ENCODER=ON -DWITH_ASF=ON -DWITH_MP4=ON -DBUILD_EXAMPLES=ON 2>&1 | tee j_cmake
make -j9 && make install 
if [ "$?" != "0" ]
then
