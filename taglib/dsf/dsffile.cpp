...
//JBH ==========================================================================<
  ...
  long lengthSeekPosition = tell();   //JBH: save the current seek position of length
...
#define JBH_DSF_CHECK_FILESIZE //JBH: Do not define for now. Too much strict rule is applied and lots of files will not be read.
  ...
  if(d->fileSize != fileLength) {
     ...
     */
    seek(lengthSeekPosition);
    curSeekPosition = tell();   //JBH debug
    long fileSizeBE = readBlock(8).toLongLong(true); //JBH: try BigEndian
    if(fileSizeBE == fileLength) {
      d->fileSize = fileSizeBE;
      isBigEndian = true;
    }
    curSeekPosition = tell();   //JBH debug
  }
