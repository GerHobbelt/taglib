...
bool DSDIFF::File::isSupported(IOStream *stream) //JBH FIXME
{
...
void DSDIFF::File::read(bool readProperties, Properties::ReadStyle propertiesStyle)
{
  ...
  d->type = readBlock(4);
  d->size = readBlock(8).toLongLong(bigEndian); //JBH: Note "true" for DFF, "false" for DSF. DFF uses the BigEndian, while DSF uses the LittleEndian. 
  d->format = readBlock(4);

  // + 12: chunk header at least, fix for additional junk bytes
  while(tell() + 12 <= length()) {
    ByteVector chunkName = readBlock(4); //JBH: 6 chunks in dff: "FVER", "PROP", "DSD ", "DIIN", "COMT", "ID3 "
    //debug("DSDIFF::File::read() -- chunk found:" + chunkName); //JBH
...
//JBH ==========================================================================>
  ...
  for(unsigned int i = 0; i < d->chunks.size(); i++) {
    //debug("DSDIFF::File::read() -- process chunk:" + d->chunks[i].name); //JBH
...
//JBH ==========================================================================>
        ...
        Chunk64 chunk;
        chunk.name = propChunkName; //JBH: PROP child chunks: "FS  ", "CHNL", "CMPR", "LSCO"
      ...
      /*
       *JBH: The "dff(dsdiff)" file may have tags spreadedly over "two" types of tagging:
       *     - TagLib::ID3v2::Tag
       *     - TagLib::DSDIFF::DIIN::Tag
       *     So, make sure handling those two tags in the client (the scanner).
       */
...
//JBH ==========================================================================>
        ...
        Chunk64 chunk;
        chunk.name = diinChunkName; //JBH: DIIN child chunks: "MARK", "DIAR", "DITI"
      ...
      /*
       *JBH NOTE: In DFF, the ID3 tag could be in 2 chunks:
       *          - in the ID3 "subchunk" of the 'PROP' chunk
       *          - or
       *          - in                       the 'ID3 ' chunk
       */
    ...
    //JBH <
    else if(d->chunks[i].name == "FVER") {
      int cool =7; //do nothing for the FVER chunk for now
    }
    else if(d->chunks[i].name == "COMT") {
      int dool =7; //do nothing for the COMT chunk for now
    }
    else {
        debug("DSDIFF::File::read() -- ignore invalid chunk name:" + d->chunks[i].name);
//JBH ==========================================================================<
  #ifdef _WIN32
        debug("JBH: DSF file: " + static_cast<FileName>(name()).toString()); //JBH add
  #else
...
//JBH ==========================================================================<
  /*
   *JBH: dstEncoded TRUE:  DST Encoded audio data
   *     dstEncoded FALSE: Uncompressed, plain DSD audio data
   *     Aurender does not support DST-encoded for now
   */
  bool dstEncoded = false;
//JBH ==========================================================================>

  /*
   *JBH: a refrence from dff-jbh
  ...
  for(unsigned int i = 0; i < d->childChunks[PROPChunk].size(); i++) {
    //debug("DSDIFF::File::read() -- process PROP child chunk:" + d->childChunks[PROPChunk][i].name); //JBH
...
//JBH ==========================================================================<
    else if(d->childChunks[PROPChunk][i].name == "CMPR") {
      //JBH FIXME: verify the logic.
      seek(d->childChunks[PROPChunk][i].offset);
      ByteVector compressType = readBlock(4);
      if (compressType == "DST ") {
        dstEncoded = true;
      }
      else {
    ...
    else if(d->childChunks[PROPChunk][i].name == "ABSS") {
      int kool = 7; //JBH NOTE: not use for now
    }
    else if(d->childChunks[PROPChunk][i].name == "LSCO") {
      int mool = 7; //JBH NOTE: not use for now
    }
    else {
      debug("DSDIFF::File::read() -- ignore invalid PROP child chunk name:" + d->childChunks[PROPChunk][i].name);
//JBH ==========================================================================<
  #ifdef _WIN32
      debug("JBH: DSF file: " + static_cast<FileName>(name()).toString()); //JBH add
  #else
...
//JBH ==========================================================================>
  ...
  if(d->hasDiin) {
    for(unsigned int i = 0; i < d->childChunks[DIINChunk].size(); i++) {
      //debug("DSDIFF::File::read() -- process DIIN child chunk:" + d->childChunks[DIINChunk][i].name); //JBH
      ...
      //JBH <
      else if(d->childChunks[DIINChunk][i].name == "MARK") {
        int pool = 7; //JBH NOTE: not use for now
      }
      else {
        debug("DSDIFF::File::read() -- ignore invalid DIIN child chunk name:" + d->childChunks[DIINChunk][i].name);
//JBH ==========================================================================<
  #ifdef _WIN32
        debug("JBH: DSF file: " + static_cast<FileName>(name()).toString()); //JBH add
  #else
