...
class DSF::Header::HeaderPrivate : public TagLib::RefCounter
{
public:
  HeaderPrivate() :
    isValid(false),
...
DSF::Header::Header(const TagLib::ByteVector &data)
{
  d = new HeaderPrivate;
  parse(data);
}

DSF::Header::Header(const DSF::Header &h) : d(h.d)
{
  d->ref();
}
...
DSF::Header &DSF::Header::operator=(const DSF::Header &h)
{
  if(&h == this)
    return *this;
...
void DSF::Header::parse(const TagLib::ByteVector &data)
{
  if (data.size() < DSD_HEADER_SIZE + FMT_HEADER_SIZE) {
    debug("DSF::Header::parse(): header size incorrect");
    return;
  }
